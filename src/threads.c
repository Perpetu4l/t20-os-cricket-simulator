...
void *batsman_thread(void *arg)
{
    ...
    while (match.match_running)
    {
        ...
        int is_legal = (original_result != 7);
        ...
        if (result == -1) {
            ...
            if (!deadlock_happened && !fielder_runout)
                b->wickets++;
        ...
        if (is_legal) {
            ...
            if (deadlock_happened) {
                ...
                printf("  [UMPIRE/DEADLOCK] Circular wait — %s is run out\n",
                       batsmen[victim].name);
                pthread_mutex_unlock(&print_mutex);
            } else if (fielder_runout) {
                pthread_mutex_lock(&print_mutex);
                printf("  [FIELDER %d] direct hit — stumps broken!\n", saved_fielder_id);
                printf("  [UMPIRE/FIELDER]  RUN OUT! %s is dismissed\n",
                       batsmen[victim].name);
                pthread_mutex_unlock(&print_mutex);
            }
